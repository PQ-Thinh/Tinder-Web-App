...
function formatHobbies(userHobbies: RawUserHobby[] | null | undefined): Hobby[] {
...
export async function getPotentialMatches(): Promise<UserProfile[]> {
    ...
    const userPrefs = (prefsResponse.data?.preferences || {}) as UserPreferences; const likedUserIds = (likesResponse.data || []).map((l) => l.to_user_id);
...
export async function likeUser(toUserId: string) {
    ...
    if (reverseLike) {
        ...
        if (matchedUser) {
            const { latitude, longitude } = parsePostGISLocation(matchedUser.location);
