...
const NavLink = ({
  ...
  if (mobile) {
    return (
      ...
      >
        ...
        {isActive && (
          <div
            className={`absolute right-4 w-2 h-2 rounded-full bg-gradient-to-r ${activeGradient}`}
          ></div>
        )}
      </Link>
    );
  ...
  return (
    <Link
      href={href}
      onClick={onClick}
      className={`flex items-center gap-2 px-4 py-2 rounded-2xl transition-all duration-300 group ${
        isActive
      ...
      <div className="container mx-auto px-6">
        <div className="flex items-center justify-between h-20">
          ...
          >
            ...
            <span
              className="text-xl font-extrabold bg-gradient-to-r from-[#db2777] via-[#f97316] to-[#db2777] bg-clip-text text-transparent tracking-tight bg-[length:200%_auto]"
              style={{
            ...
            >
              <style jsx>{`
                @keyframes shimmer {
                  0% {
                    background-position: 200% center; /* Bắt đầu từ bên trái */
                  }
                  100% {
                    background-position: 0% center; /* Chạy về bên phải */
                  }
                }
              `}</style>
              StreamMatch
