...
// --- 1. ĐỊNH NGHĨA LIST ICON & MENU ITEMS ---
const NAV_ITEMS = [
  {
    label: "Khám Phá",
    href: "/matches",
    activeGradient: "from-orange-500 to-red-500",
    hoverText: "group-hover:text-orange-500", // Màu khi hover
    icon: (
      <svg
        className="w-6 h-6"
        fill="none"
  ...
  },
  {
    ...
    hoverText: "group-hover:text-pink-500", // Màu khi hover
    icon: (
      <svg
        className="w-6 h-6"
        fill="none"
  ...
  },
  {
    ...
    hoverText: "group-hover:text-blue-500", // Màu khi hover
    icon: (
      <svg
        className="w-6 h-6"
        fill="none"
  ...
  },
  {
    ...
    hoverText: "group-hover:text-green-500", // Màu khi hover
    icon: (
      <svg
        className="w-6 h-6"
        fill="none"
...
const NavLink = ({
  ...
  if (mobile) {
    return (
      ...
      >
        <div
          className={`p-2.5 rounded-xl transition-all duration-300 shadow-sm relative ${
            isActive
        ...
        <span
          className={`text-lg font-bold transition-colors duration-300 ${
            isActive
              ...
              : `text-gray-700 dark:text-gray-200 ${hoverText}` // Thêm hoverText vào đây
          }`}
        >
          {label}
        </span>
  ...
  return (
    <Link
      href={href}
      onClick={onClick}
      className={`flex items-center gap-2 px-4 py-2 rounded-2xl transition-all duration-300 group relative ${
        isActive
    ...
    >
      ...
      <span
        className={`font-bold text-sm xl:text-base transition-colors duration-300 ${
          isActive
            ? `text-transparent bg-clip-text bg-gradient-to-r ${activeGradient}`
            : isScrolled
            ? `text-gray-600 dark:text-gray-300 ${hoverText}` // Khi cuộn: Xám -> Màu Hover
            : `text-gray-900 dark:text-white drop-shadow-sm ${hoverText}` // Khi trong suốt: Đen/Trắng -> Màu Hover (KHÔNG PHẢI MÀU ĐEN)
        }`}
      >
        {label}
      </span>
          ...
          {user && (
            <>
              ...
              >
                ...
                <span className="text-lg font-bold dark:text-gray-200 group-hover:text-red-500">
                  Đăng xuất
                </span>
              </button>
            </>
